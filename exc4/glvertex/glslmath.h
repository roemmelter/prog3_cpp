...
inline vec2 lerp(double w, const vec2 &a,const vec2 &b)
   {return((1-w)*a+w*b);}

//! linear interpolation (GLSL-style)
inline vec2 mix(const vec2 &a,const vec2 &b, double w)
   {return((1-w)*a+w*b);}
...
//! * primarily use vec2 for linear math calculations
class vec2f
   {
   public:
...
//! linear interpolation
inline vec3 lerp(double w, const vec3 &a,const vec3 &b)
   {return((1-w)*a+w*b);}

//! linear interpolation (GLSL-style)
inline vec3 mix(const vec3 &a,const vec3 &b, double w)
   {return((1-w)*a+w*b);}
...
//! * access to components x/y/z via . component selector
//! * this vector type is designed for compact storage purposes
//! * primarily use vec3 for linear math calculations
class vec3f
   {
   public:
...
//! linear interpolation
inline vec4 lerp(double w, const vec4 &a,const vec4 &b)
   {return((1-w)*a+w*b);}

//! linear interpolation (GLSL-style)
inline vec4 mix(const vec4 &a,const vec4 &b, double w)
   {return((1-w)*a+w*b);}
...
//! * access to components x/y/z/w via . component selector
//! * this vector type is designed for compact storage purposes
//! * primarily use vec4 for linear math calculations
class vec4f
   {
   public:
...
class mat3
   ...
   static mat3 rotate(double angle,double vx,double vy,double vz)
      {
      return(rotate(angle,vec3(vx,vy,vz)));
      }
   ...
   static mat3 rotate(const vec3 &from,const vec3 &to)
      ...
      vec3 d1=from.normalize();
      vec3 d2=to.normalize();
      axis=d1.cross(d2);

      double l=axis.length();
      c=sqrt(1-l*l); // cos(angle);
      s=l; // sin(angle)
...
//! * this matrix type is not designed for calculations
//! * it is designed for compact storage and transfer purposes only
//!  * use mat3 for linear math calculations, then copy to mat3f and
...
class mat4
   ...
   static mat4 rotate(double angle,double vx,double vy,double vz)
      {return(mat3::rotate(angle,vx,vy,vz));}
...
//! inline rotate
inline mat4 rotate(const mat4 &m,const vec3 &from,const vec3 &to)
   {return(m*mat4::rotate(from,to));}

//! inline determinant
inline double determinant(const mat4 &m)
   {return(m.det());}
...
//! create linear transformation matrix
inline mat4 mat4::transform(const vec3 &from,const vec3 &to,const vec3 &up,bool scale)
   {
   vec3 d = (to-from).normalize();
   vec3 r = up.cross(d).normalize();
   vec3 u = d.cross(r).normalize();

   if (scale) d *= (to-from).length();

   return(mat4::transform(0.5*(from+to),r,u,d));
   }
...
//! * this matrix type is not designed for calculations
//! * it is designed for compact storage and transfer purposes only
//!  * use mat4 for linear math calculations, then copy to mat4f and
//!  * cast to const float * to transfer via glUniform:
...
inline vec3 rgb2hsv(double r,double g,double b)
   {
   double h,s,v;
   double maxv,minv,diff,rdist,gdist,bdist;
   ...
   if (s==0.0) h=0.0;
      ...
      else if (g==maxv) h=2.0+rdist-bdist;
      else h=4.0+gdist-rdist;
...
//! hsv to rgb conversion
inline vec3 hsv2rgb(vec3 hsv)
   {return(hsv2rgb(hsv.x,hsv.y,hsv.z));}

//! rgb to hsv conversion
inline vec3 rgb2hsv(vec3 rgb)
   {return(rgb2hsv(rgb.x,rgb.y,rgb.z));}
...
inline double distance2ray(vec3 p,vec3 o,vec3 d)
   {
   d=d.normalize();
...
inline double distance2ray2(vec3 p,vec3 o,vec3 d)
   {
   d=d.normalize();
...
inline double distance2line2(vec3 p,vec3 a,vec3 b)
   {
   vec3 n=b-a;
   n=n.normalize();
...
inline int vtest_plane_point(const vec3 &o,const vec3 &n,
                             const vec3 &point)
   {
   return(dot(point-o,n)>0);
   }

//! bounding sphere visibility test
inline int vtest_plane_sphere(const vec3 &o,const vec3 &n,
                              const vec3 &center,double radius)
   {
   return(dot(center-o,n)>-radius);
   }

//! bounding box visibility test
inline int vtest_plane_bbox(const vec3 &o,const vec3 &n,
                            const vec3 &center,const vec3 &extent)
   {
   return(dot(center-o,n)>-0.5*dot(extent,vec3(fabs(n.x),fabs(n.y),fabs(n.z))));
   }
...
inline int test()
   ...
   // test vectors
   {
      ...
      if (hsv2rgb(60,1,1)!=vec3(1,1,0)) errors++;
      if (rgb2hsv(1,1,0)!=vec3(60,1,1)) errors++;
   }
   ...
   // test transformations
   {
      ...
      mat4 fromto=mat4::rotate(vec3(0,1,0),vec3(1,0,0));
      if (!(fromto*vec4(0,1,0)==vec4(1,0,0))) errors++;
   ...
   {
      mat4 T=mat4::translate(0,0,1)*mat4::rotate(90,vec3(0,1,0))*mat4::scale(0.5);
      vec4 v(2,0,0);
      v=T*v;
