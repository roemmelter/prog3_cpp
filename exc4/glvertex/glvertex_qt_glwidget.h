...
//! \file
//! LGL Qt widget
#ifndef GLVERTEX_QT_GLWIDGET_H
#define GLVERTEX_QT_GLWIDGET_H
...
#if (QT_VERSION>=QT_VERSION_CHECK(5, 9, 0)) && !defined(LGL_GLES)
#   define LGL_USE_QOPENGLWIDGET
#endif

#ifndef LGL_USE_QOPENGLWIDGET
#   include <QGLWidget>
#   define LGL_QGLWIDGET QGLWidget
#else
#   include <QOpenGLWidget>
#   define LGL_QGLWIDGET QOpenGLWidget
#endif

//! Qt OpenGL widget base class
class lgl_Qt_GLWidget: public LGL_QGLWIDGET
{
public:

   //! default ctor
   lgl_Qt_GLWidget(QWidget *parent = NULL,
                   bool core = true)
      : LGL_QGLWIDGET(parent),
        core_(core),
        time_(0),
        speedup_(1),
        background_(false),
...
#ifndef LGL_USE_QOPENGLWIDGET
      QGLFormat format(QGL::DoubleBuffer | QGL::DepthBuffer);
...
#if (LGL_OPENGL_VERSION >= 30)
      std::cerr << "Creating QGLWidget with"
                << std::endl
                << " opengl "
#ifndef LGL_USE_QOPENGLWIDGET
                << (format.profile()==QGLFormat::CoreProfile? "core" : "compatibility")
#else
